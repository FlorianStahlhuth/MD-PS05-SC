...
-   In this regression table the intercept `r round(get_regression_table(hate_mod)$estimate[1], 3)` corresponds to the mean number of hate crimes per 100k individuals recorded in the 10 days after the 2016 election in states with high Trump support. This mean is `r round(mean(subset(hate_crimes, trump_support == "high")$hate_crimes, na.rm = TRUE), 3)`.
:::
